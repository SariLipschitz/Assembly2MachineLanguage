...
/*
@author: SARI LIPSCHITZ

my files: (.c)

linked_list.c all the lists function-The lists are built to be that each part of the list is a union of structs(macro /label / print_line / 	instructions...) and this union struct is used for evry list in my program
...
bit.c convers the data to binary
...
global variabels:(4)
label_list, print_list, line_num, is_valid

------------------------------------------------program steps(run.c)-------------------------------------------------------------

step1: loops the users entered files that was sent as argoments
step2: If the desired file is present, the program will first open the file and send it to the "turnToAm" function, this function will 	deploy the macros and create a new file(.am) . If everything went well from here the program will only use the "after macro file(.am)"
...
Therefore, you can start reading the file from the beginning and encode it. "turnToBit" function will read each line and With the help of the lists from step 3 and other functions will collect the information needed for encoding(command opcode,value of immidiat/register, address of labels) 

step5: in step 3 and step 4 any printing data was add into a print list with a type of this print line (error/entry/exter/binary)
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		turnToAm(f, file_name, line, first_word_in_line);
		fclose(f);
		...
		if (is_valid != 1)
			turnToBit(f, line, first_word_in_line);
		 ...
		 * files the externy and entry labels and will decode the file
		 */
		printings(file_name);
